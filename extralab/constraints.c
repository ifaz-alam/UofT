...
struct constraints *init_constraints() {
    struct constraints *struct_to_return = malloc(sizeof(struct constraints) * 1);

    // Set the fields to 0 or empty string
...
void set_green(char letter, int index, struct constraints *con) {
    ...
    char a_letter[1] = {letter};
    strncat(con->must_be[index], a_letter, SIZE - strlen(con->must_be[index]));
...
void set_yellow(int index, char *cur_tiles, char *next_tiles,
    ...
    for (int i=0; i<WORDLEN; i++) {
        if (next_tiles[i] == 'y' || (next_tiles[i] == 'g' && cur_tiles[i] != 'g' && (i != index || next_tiles[index] == '-'))) {
