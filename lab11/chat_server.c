...
int read_from(int client_index, struct sockname *users) {
    ...
    if (users[client_index].username == NULL && num_read != 0) {
        ...
        strncpy(users[client_index].username, buf, strlen(buf));
        users[client_index].username[strlen(buf) - 1] = '\0';
        return 0;
    }

    char msg[2 * BUF_SIZE + 2];
    // has garbage memory so reset it
    memset(msg, '\0', 2 * BUF_SIZE + 2);
    // the null terminator exists so this is safe
    strcat(msg, users[client_index].username);
    strcat(msg, ": ");
    strcat(msg, buf);

    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        // only send to sockets that arent -1
        if (users[i].sock_fd != -1) {
            // it must be set if its not -1, so its a user that can receive info
            if (num_read == 0 || write(users[i].sock_fd, msg, strlen(msg)) != strlen(msg)) {
                // TA said to ignore this part
                users[i].sock_fd = -1;
                // close the connection to whoever sent it to us
                return fd;
            }
        }
    }
...
int main(void) {
    ...
    while (1) {
        ...
        // for (int index = 0; index < MAX_CONNECTIONS; index++) {
        //     if (client[index].sock_fd > -1 && FD_ISSET(client[index].sock_fd, &listen_fds)) {
